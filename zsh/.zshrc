...
if [[ -z "$TMUX" ]] && [[ -n "$ITERM_SESSION_ID" || "$TERM_PROGRAM" == "ghostty" ]]; then
...
if [[ -n "$ITERM_SESSION_ID" || "$TERM_PROGRAM" == "ghostty" ]]; then
...
export PATH=$PATH:/usr/bin:/usr/local/go/bin:~/.local/bin:~/.local/bin/
